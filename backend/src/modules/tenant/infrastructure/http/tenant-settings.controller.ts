import { Controller, Get, Put, Body, HttpCode, HttpStatus, UseGuards, Request } from '@nestjs/common';
...
export class TenantSettingsController {
  ...
  @Get('password-policy')
  @ApiOperation({ summary: 'RF-1.05: Obtener política de caducidad de contraseña del tenant' })
  @ApiResponse({ status: 200, type: PasswordPolicyResponseDto })
  @ApiResponse({ status: 401, description: 'No autenticado' })
  async getPolicy(@Request() req: any): Promise<PasswordPolicyResponseDto> {
    return this.getPasswordPolicy.execute(req.user.tenantId);
  }

  @Put('password-policy')
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'RF-1.05: Actualizar política de caducidad de contraseña del tenant' })
  @ApiResponse({ status: 200, type: PasswordPolicyResponseDto })
  @ApiResponse({ status: 401, description: 'No autenticado' })
