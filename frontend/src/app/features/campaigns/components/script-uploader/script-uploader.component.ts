...
@Component({
  ...
  template: `
    <div class="uploader-container">
      <h3>Guion de la Campaña (TXT)</h3>
      <p>Sube un archivo de texto plano (.txt) con el guion que seguirá el agente.</p>
      
      <div class="upload-area" (click)="fileInput.click()">
        <mat-icon color="accent">description</mat-icon>
        <span>Haz clic para seleccionar el archivo TXT</span>
        <input #fileInput type="file" accept=".txt" style="display: none" (change)="onFileSelected($event)">
      </div>
      ...
      <div class="actions" *ngIf="selectedFile()">
        <button mat-raised-button color="accent" [disabled]="isUploading()" (click)="upload()">
          Subir Guion
        </button>
      </div>
  ...
  styles: [`
    ...
    .upload-area:hover { background: #f9f9f9; border-color: #ff4081; }
